...
[![Docker Hub](https://img.shields.io/badge/docker-ready-blue.svg)](https://hub.docker.com/r/rfariasc/docker-airflow/)
[![Docker Pulls](https://img.shields.io/docker/pulls/rfariasc/docker-airflow.svg)]()
[![Docker Stars](https://img.shields.io/docker/stars/rfariasc/docker-airflow.svg)]()

This repository contains **Dockerfile** of [apache-airflow](https://github.com/apache/incubator-airflow) for [Docker](https://www.docker.com/)'s [automated build](https://registry.hub.docker.com/repository/docker/rfariasc/docker-airflow) published to the public [Docker Hub Registry](https://registry.hub.docker.com/).
...
Pull the image from the Docker repository.
    ...
    docker pull rfariasc/docker-airflow

## Usage

Should have kubernetes running on docker althought can be adapted to use with whatever cluster you have.
...
Optionally install [Extra Airflow Packages](https://airflow.incubator.apache.org/installation.html#extra-package) and/or python dependencies at build time :
    ...
    docker build --rm --build-arg AIRFLOW_DEPS="datadog,dask" -t rfariasc/docker-airflow .
    docker build --rm --build-arg PYTHON_DEPS="flask_oauthlib>=0.9" -t rfariasc/docker-airflow .

or combined

    docker build --rm --build-arg AIRFLOW_DEPS="datadog,dask" --build-arg PYTHON_DEPS="flask_oauthlib>=0.9" -t rfariasc/docker-airflow .

Don't forget to update the airflow images in the docker-compose files to rfariasc/docker-airflow:latest.

## Usage

By default, docker-airflow runs Airflow with **SequentialExecutor** :

    docker run -d -p 8080:8080 rfariasc/docker-airflow webserver
...
`LOAD_EX=n`
    ...
    docker run -d -p 8080:8080 -e LOAD_EX=y rfariasc/docker-airflow
...
For encrypted connection passwords (in Local or Celery Executor), you must have the same fernet_key. By default docker-airflow generates the fernet_key at startup, you have to set an environment variable in the docker-compose (ie: docker-compose-LocalExecutor.yml) file to set the same key accross containers. To generate a fernet_key :
    ...
    docker run rfariasc/docker-airflow python -c "from cryptography.fernet import Fernet; FERNET_KEY = Fernet.generate_key().decode(); print(FERNET_KEY)"
...
If you want to run other airflow sub-commands, such as `list_dags` or `clear` you can do so like this:
    ...
    docker run --rm -ti rfariasc/docker-airflow airflow list_dags
...
You can also use this to run a bash shell or any other command in the same environment that airflow would be run in:
    ...
    docker run --rm -ti rfariasc/docker-airflow bash
    docker run --rm -ti rfariasc/docker-airflow ipython
